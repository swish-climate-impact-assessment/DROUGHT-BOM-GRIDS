...
#+begin_src R :session *R* :tangle no :exports none :eval no
  ...
  dbSendQuery(ch, "grant usage on schema bom_grids to ivan_hanigan")
  
  dir("data")
  args(load2postgres)
  load2postgres("data/drought_bom_grids.grid_nsw.csv", "bom_grids",
                "grid_nsw", pguser = "postgres",
                db = "django", ip = "localhost", printcopy = F)
  dbSendQuery(ch, "grant all on table bom_grids.grid_nsw to ivan_hanigan")
...
#+begin_src R :session *R* :tangle code/add metadata using EML.R :exports none :eval no
  #### name:add metadata using EML ####
  library(devtools)
